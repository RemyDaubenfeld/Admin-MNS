...
const passwordInfoMessage = "Le mot de passe doit comporter entre 8 et 40 caractères et contenir au minimum une minuscule, une majuscule, un chiffre et un caractère spécial ( # , ? , ! , @ , $ , % , ^ , & , * , - ).";
const passwordLengthMessage = "Le mot de passe doit comporter entre 8 et 40 caractères.";
const passwordFormatMessage = "Le mot de passe doit contenir au minimum : une minuscule, une majuscule, un chiffre et un caractère spécial ( # , ? , ! , @ , $ , % , ^ , & , * , - ).";
const passwordRequiredMessage = "Le champ mot de passe est obligatoire.";

const confirmPasswordInfoMessage = "Les deux mots de passe doivent correspondrent.";
const confirmPasswordDifferentMessage = "Les deux mots de passe doivent correspondrent.";
const confirmPasswordRequireMessage = "Le champ de confirmation du mot de passe est obligatoire.";
...
function regexPasswordStrong(password) {
    return /^(?=.*?[A-Z]{2})(?=.*?[a-z]{2})(?=.*?[0-9]{2})(?=.*?[#?!@$%^&*-]{2}).{12,}$/.test(password);
}

function regexPasswordStronger(password) {
    return /^(?=.*?[A-Z]{3})(?=.*?[a-z]{3})(?=.*?[0-9]{3})(?=.*?[#?!@$%^&*-]{3}).{16,}$/.test(password);
}
...
export function passwordCheckStrength(passwordId) {
    const password = document.querySelector(`#${passwordId}`);
    const passwordStrengthLabel = document.querySelector(`#${passwordId}StrengthLabel`);
...
export function confirmPasswordCheck(confirmPasswordId, validatePasswordId, submit) {
...
export function formCheck(formId, fields) {
    ...
    formSubmit.addEventListener("click", async function (e) {
        ...
        const errorsCount = errors > 1 ? `${errors} champs invalides` : `${errors} champ invalide`;

        if (!validForm) {
            addModalMessage("warning", `Il y a ${errorsCount} dans votre formulaire.`);
            return
        }
