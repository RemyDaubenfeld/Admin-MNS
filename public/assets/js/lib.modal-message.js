...
export function addModalMessage(type, message) {
    ...
    fetch("scripts.php?script=add-modal-message", {
        method: "POST",
        headers: {
...
function addModalMessageDOM(index, type, message) {
    ...
    const modalMessageIcon = document.createElementNS('http://www.w3.org/2000/svg', 'svg');
    ...
    const modalMessageIconPath = document.createElementNS('http://www.w3.org/2000/svg', 'path');
    ...
    const modalMessageClose = document.createElementNS('http://www.w3.org/2000/svg', 'svg');
    modalMessageClose.setAttribute('xmlns', 'http://www.w3.org/2000/svg');
    modalMessageClose.setAttribute('viewBox', '0 0 512 512');
    modalMessageBox.id = `newCloseModalMessage${index}`;
    ...
    const modalMessageClosePath = document.createElementNS('http://www.w3.org/2000/svg', 'path');
    modalMessageClosePath.setAttribute('d', "M342.6 150.6c12.5-12.5 12.5-32.8 0-45.3s-32.8-12.5-45.3 0L192 210.7 86.6 105.4c-12.5-12.5-32.8-12.5-45.3 0s-12.5 32.8 0 45.3L146.7 256 41.4 361.4c-12.5 12.5-12.5 32.8 0 45.3s32.8 12.5 45.3 0L192 301.3 297.4 406.6c12.5 12.5 32.8 12.5 45.3 0s12.5-32.8 0-45.3L237.3 256 342.6 150.6z");
...
export function closeModalMessages() {
    ...
    modalMessages.forEach(modalMessage => {
        ...
        const closeModalMessage = document.querySelector(`#close${ucFirst(modalMessage.id)}`);
...
function removeModalMessage(modalMessage, timeout) {
    setTimeout(() => {
        ...
        fetch("scripts.php?script=delete-modal-message", {
            method: "POST",
            headers: {
                "Content-Type": "application/json"
            },
            body: JSON.stringify({ modal_message_index: onlyNumber(modalMessage.id) })
        })
        .then(response => response.json())
        .then(data => {
